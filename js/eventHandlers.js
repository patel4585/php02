			...
			if (xhr.readyState == 4 && xhr.status == 200) {
				...
				if (loginHistoryArray.length > 0) {
					...
					if (username == usernameFromBackendData) {
						...
						let loginTime = jsonObject.timestamp;


						// TODO 6c: create p tag for each loginTime and append that tag as a child of the lastLoginDiv.  
						// let p = document.createElement("p");
						// let text = document.createTextNode("Timestamp: " + loginTime)
						// lastLoginDiv.appendChild(p);
						// p.append(text);

						//Both code works

						let p = document.createElement("p");
						p.textContent = "Timestamp: " + loginTime;
						lastLoginDiv.append(p);
